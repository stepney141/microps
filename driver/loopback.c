...
struct loopback {  // private data for the loopback device driver
    int irq;
    mutex_t mutex;
    struct queue_head queue;
};

struct loopback_queue_entry {  // queue entry for the loopback device driver
    uint16_t type;
    size_t len;
...
static int loopback_transmit(struct net_device *dev, uint16_t type,
    ...
    mutex_lock(&PRIV(dev)->mutex);  // Protect the queue with a mutex
    if (PRIV(dev)->queue.num >= LOOPBACK_QUEUE_LIMIT) {  // Check the queue size
        mutex_unlock(&PRIV(dev)->mutex);
        errorf("queue is full");
        return -1;
    }

    // Allocate a queue entry and push it to the queue
    ...
    // Copy the packet data to the queue entry
    entry->type = type;
    entry->len = len;
    ...
    intr_raise_irq(PRIV(dev)->irq);  // Notify the arrival of a packet by IRQ
    return 0;
}
...
struct net_device *loopback_init(void) {
    ...
    dev = net_device_alloc();  // Generate a loopback device
    if (!dev) {
    ...
    mutex_init(&lo->mutex);
    queue_init(&lo->queue);
    dev->priv = lo;  // Set the private data
